...
@OnlyIn(Dist.CLIENT)
public class ScreenChocoboChest extends ScreenBase<MenuChocoboChest> {
	...
	public ScreenChocoboChest(MenuChocoboChest container, Inventory player, Component name) {
	...
	protected void renderLabels(GuiGraphics matrix, int mouseX, int mouseY){
		int x = (this.width  - 176) / 2;
		int y = (this.height - 178) / 2;
		String text = "CHOCO NAME";
		int w = this.font.width(text) / 2;
		matrix.drawString(font, text, x + 49 - w, y + 10, 16777215);
	}
